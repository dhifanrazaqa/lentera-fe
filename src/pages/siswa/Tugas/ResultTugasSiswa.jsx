  ...
  useEffect(() => {
    if (assignmentId && classId && contentId) {
      const fetchAssignmentData = async () => {
        setIsLoading(true);
        try {
          const [assignmentData, headerData, submissionData] =
            await Promise.all([
      ...
      <div className="bg-white rounded-lg flex flex-col p-5">
        ...
        <form>
          <div>
            ...
            <p className="font-bold text-xl mt-4 mb-4 text-green-500">
              {submissionData?.data?.grade === null ? "Belum Dinilai" : submissionData?.data?.grade}
            </p>
              ...
              <div className="p-4 pl-[20%] pr-[20%]">
                <div
                  dangerouslySetInnerHTML={{
                    __html: submissionData?.data?.text,
                  }}
                />
              </div>
            </div>
