...
const submissionSchema = z.array(
  z.object({
    ...
    value: z
      ...
      .refine((val) => val >= 1 && val <= 100, 'Grade must be between 1 and 100'), 
  })
);
...
export default function DetailSubmission() {
  ...
  useEffect(() => {
    if (usersClass?.data) {
      const initialSubmissions = usersClass.data.map((item) => {
        if (item.user.Submission[0]?.id != null && item.user.Submission[0]?.grade == null) {
          return {
  ...
  const OnClick = async () => {
    try {
      ...
      for (let i = 0; i < submissions.length; i++) {
        ...
        console.log(`Submission ${submissions[i].id} graded successfully:`, result);
      }
    } catch (error) {
                ...
                filteredData.map((item, index) => (
                  <tr key={item.user.id} className="hover:bg-gray-50">
                    ...
                    <td className="border-b-2 border-gray-200 px-4 py-5">
                      {item.fileUrl !== null ? (
                        <div className="flex justify-center ">
                          <a className="flex rounded-md w-fit px-4 py-2 font-medium bg-gray-100 gap-2" href={item.user.Submission[0]?.fileUrl} >
                            <svg
                              width="20"
                              height="20"
                              viewBox="0 0 20 20"
                    ...
                    <td
                      ...
                      onClick={() => handleNameClick(item.user.Submission[0]?.text)} // Call handleNameClick with the user's name
                    >
                         <div dangerouslySetInnerHTML={{ __html: safeHTML(item.user.Submission[0]?.text) }} />
                    </td>
                    ...
                    <td className="flex items-center justify-center">
                        {item.user.Submission[0]?.grade != null ? (
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleInputChange(item.user.Submission[0]?.id, e.target.value)
                            }
                            ...
                            disabled={item.user.Submission[0]?.id == null || item.user.Submission[0]?.grade != null}
                          />
                        )}
                      </td>
                    </td>
